...
/* ESTE COM AS ÁRVORES JÁ ESTÁ, FALTA OS FREES (MAS N SEI SE VOU FAZER ISSO...) */
...
typedef struct reserve_tree_node 
{
    Reserve value;
    struct reserve_tree_node *l;
    struct reserve_tree_node *r;
    int height;
} * tree_link;

typedef struct reserve_tree_node Tree_node;
...
Reserve copy_reserve(Reserve reserve_1, Reserve reserve_2)
{
    strcpy(reserve_2.flight_ID, reserve_1.flight_ID);
    reserve_2.date.day = reserve_1.date.day;
    reserve_2.date.month = reserve_1.date.month;
    reserve_2.date.year = reserve_1.date.year;
    strcpy(reserve_2.flight_ID, reserve_1.flight_ID);
    reserve_2.num_passengers = reserve_1.num_passengers;
...
tree_link in_reserve(tree_link head_aux, char reserve_ID[])
{
    if (head_aux == NULL)
        return NULL;
    if (strcmp(reserve_ID, head_aux->value.reserve_ID) == 0)
        return head_aux;
    if (less_strings(reserve_ID, head_aux->value.reserve_ID))
        return in_reserve(head_aux->l, reserve_ID);
    else
        return in_reserve(head_aux->r, reserve_ID);
}
...
tree_link NEW_tree(Reserve reserve_aux, tree_link l, tree_link r)
{
    tree_link x = (tree_link)malloc(sizeof(Tree_node));
    ...
    x->value.reserve_ID = (char *)malloc(sizeof(char) * (strlen(reserve_aux.reserve_ID) + 1));
    ...
    x->value.date.day = reserve_aux.date.day;
    x->value.date.month = reserve_aux.date.month;
    x->value.date.year = reserve_aux.date.year;
    strcpy(x->value.flight_ID, reserve_aux.flight_ID);
    strcpy(x->value.reserve_ID, reserve_aux.reserve_ID);
    x->value.num_passengers = reserve_aux.num_passengers;
    x->l = l;
    x->r = r;
    x->height = 1;
...
int count(tree_link head_aux)
{
    if (head == NULL)
        return 0;
    else
        return count(head_aux->r) + count(head_aux->l) + 1;
}

int height(tree_link head_aux)
{
    if (head_aux == NULL)
        return 0;
    return head_aux->height;
}

void visit(Reserve reserve_aux, char flight_ID[], Date date_aux)
{
    if (strcmp(reserve_aux.flight_ID, flight_ID) == 0 && same_day(reserve_aux.date, date_aux))
        printf("%s %d\n", reserve_aux.reserve_ID, reserve_aux.num_passengers);
}

void traverse_in_order(tree_link head_aux, char flight_ID[], Date date_aux)
{
    if (head_aux == NULL)
        return;
    traverse_in_order(head_aux->l, flight_ID, date_aux);
    visit(head_aux->value, flight_ID, date_aux);
    traverse_in_order(head_aux->r, flight_ID, date_aux);
}

tree_link rotL(tree_link head_aux)
{
    int hleft, hright, xleft, xright;
    tree_link x = head_aux->r;
    head_aux->r = x->l;
    x->l = head_aux;

    hleft = height(head_aux->l);
    hright = height(head_aux->r);

    head_aux->height = hleft > hright ? hleft + 1 : hright + 1;
...
tree_link rotR(tree_link head_aux)
{
    int hleft, hright, xleft, xright;
    tree_link x = head_aux->l;
    head_aux->l = x->r;
    x->r = head_aux;
...
tree_link rotLR (tree_link head_aux)
{
    if (head_aux == NULL)
        return head_aux;
    head_aux->l = rotL(head_aux->l);
    return rotR(head_aux);
}

tree_link rotRL (tree_link head_aux)
{
    if (head_aux == NULL)
        return head_aux;
    head_aux->r = rotR(head_aux->r);
    return rotL(head_aux);
}

int balance(tree_link head_aux)
{
    if (head_aux == NULL)
        return 0;
    return height(head_aux->l) - height(head_aux->r);
}

tree_link AVLbalance(tree_link head_aux) 
{
    int balanceFactor, hleft, hright;
    if (head_aux == NULL) 
        return head_aux;
    balanceFactor = balance(head_aux);

    if (balanceFactor > 1) 
    {
        if (balance(head_aux->l) >= 0) 
            head_aux = rotR(head_aux);
        else 
            head_aux = rotLR(head_aux);
    }
    else if(balanceFactor < -1)
    { 
        if (balance(head_aux->r) <= 0) 
            head_aux = rotL(head_aux);
        else 
            head_aux = rotRL(head_aux);
    }
    else
    {
        hleft = height(head_aux->l);
...
tree_link insertR(tree_link head_aux, Reserve reserve_aux)
{
    if (head_aux == NULL)
        return NEW_tree(reserve_aux, NULL, NULL);
    if (less_strings(reserve_aux.reserve_ID, head_aux->value.reserve_ID))
        head_aux->l = insertR(head_aux->l, reserve_aux);
    else
        head_aux->r = insertR(head_aux->r, reserve_aux);

    head_aux = AVLbalance(head_aux);

    return head_aux;
}

tree_link deleteR(tree_link head_aux, char reserve_ID[]) 
{
    if (head_aux == NULL) 
        return head_aux;
    else if (less_strings(reserve_ID, head_aux->value.reserve_ID)) 
        head_aux->l = deleteR(head_aux->l, reserve_ID);
    else if (less_strings(head_aux->value.reserve_ID, reserve_ID))
        head_aux->r = deleteR(head_aux->r, reserve_ID);
    else
    {
        if (head_aux->l != NULL && head_aux->r != NULL)
        {
            tree_link aux = max(head_aux->l);
            {Reserve x; x = copy_reserve(head_aux->value, x); head_aux->value = copy_reserve(aux->value, head_aux->value); aux->value = copy_reserve(x, aux->value) ;}
            head_aux->l = deleteR(head_aux->l, aux->value.reserve_ID);
        }
        else 
        {
            tree_link aux = head_aux;
            if (head_aux->l == NULL && head_aux->r == NULL) 
                head_aux = NULL;
            else if (head_aux->l == NULL)
                head_aux = head_aux->r;
            else 
                head_aux = head_aux->l;
        free(aux);
...
tree_link deleteR_flight(tree_link head_aux, char flight_ID[]) 
{
    if (head_aux == NULL) 
        return head_aux;
    else if (less_strings(flight_ID, head_aux->value.flight_ID)) 
        head_aux->l = deleteR_flight(head_aux->l, flight_ID);
    else if (less_strings(head_aux->value.flight_ID, flight_ID))
        head_aux->r = deleteR_flight(head_aux->r, flight_ID);
    else
    {
        if (head_aux->l != NULL && head_aux->r != NULL)
        {
            tree_link aux = max(head_aux->l);
            {Reserve x; x = copy_reserve(head_aux->value, x); head_aux->value = copy_reserve(aux->value, head_aux->value); aux->value = copy_reserve(x, aux->value) ;}
            head_aux->l = deleteR_flight(head_aux->l, aux->value.flight_ID);
        }
        else 
        {
            tree_link aux = head_aux;
...
void add_or_list_reserves()
{
    ...
    if ((next_char = getchar()) != '\n')
    {
        ...
        else if (in_reserve(tree_head, reserve.reserve_ID) != NULL)
        {
        ...
        {
            tree_head = insertR(tree_head, reserve);
            i_r++;
            free(reserve.reserve_ID);
        }                
    }   
    ...
    /* Se nenhum dos erros se verificar, listar todas as reservas relativas ao voo da reserva na data correspondente */
        ...
        traverse_in_order(tree_head, reserve.flight_ID, reserve.date);
}
...
void delete_reserve_or_flight()
{
    ...
    int i = 0, i_v_aux = 0, i_r_aux = 0, i_current = 0;
    ...
    if (strlen(reserve_buffer) < 10)
    {
        ...
        else                    
        {
            ...
            for (i = 0; i < i_r_aux; i++)
            {
                tree_head = deleteR_flight(tree_head, reserve_buffer);
                i_r--;
            }
            ...
            while (i < i_v_aux)
            {
                ...
                if (strcmp(reserve_buffer, flights[i].code) != 0)
                {
    ...
    else if (in_reserve(tree_head, reserve_buffer) == NULL)
        puts("not found");
    else
    {
        /* Voltar a adcionar os passageiros ao voo, porque este foi eliminado */
        add_passengers(reserve, i_v);
        tree_head = deleteR(tree_head, reserve_buffer);
        i_r--;
    }
}
